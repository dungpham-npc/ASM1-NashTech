...
public class UserControllerTest {
    ...
    void changePassword_ValidRequest_ReturnsSuccessResponse() throws Exception {
        ...
        when(userFacade.changePassword(any(ChangePasswordRequest.class)))
                .thenReturn(BaseResponse.build("OTP sent to your email", true));
        ...
        mockMvc.perform(post("/api/v1/users/change-password")
                ...
                .andExpect(jsonPath("$.code", is("200")))
                .andExpect(jsonPath("$.message", is("Success")))
                .andExpect(jsonPath("$.data", is("OTP sent to your email")));
    }

    @Test
    void changePassword_WithoutLogin_ReturnsUnauthorized() throws Exception {
        // Arrange
        ChangePasswordRequest request = ChangePasswordRequest.builder()
                .oldPassword("OldPassword123!")
                .newPassword("NewPassword123!")
    ...
    void changePassword_InvalidRequest_ReturnsBadRequest() throws Exception {
        // Arrange
        ChangePasswordRequest request = ChangePasswordRequest.builder()
                .oldPassword("short")  // Invalid password format
    ...
    @Test
    @WithMockUser(roles = "CUSTOMER")
    void verifyOtpAndChangePassword_ValidRequest_ReturnsSuccessResponse() throws Exception {
        // Arrange
        OtpRequest request = OtpRequest.builder()
                .otp("123456")
                .build();

        when(userFacade.verifyOtpAndChangePassword(any(OtpRequest.class)))
                .thenReturn(BaseResponse.build("Password changed successfully", true));

        // Act & Assert
        mockMvc.perform(post("/api/v1/users/verify-otp")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request))
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code", is("200")))
                .andExpect(jsonPath("$.message", is("Success")))
                .andExpect(jsonPath("$.data", is("Password changed successfully")));
    }

    @Test
    void verifyOtpAndChangePassword_WithoutLogin_ReturnsUnauthorized() throws Exception {
        // Arrange
        OtpRequest request = OtpRequest.builder()
                .otp("123456")
                .build();
    ...
    @Test
    @WithMockUser(roles = "CUSTOMER")
    void verifyOtpAndChangePassword_InvalidRequest_ReturnsBadRequest() throws Exception {
        // Arrange
        OtpRequest request = OtpRequest.builder()
                .otp("")  // Empty OTP
                .build();
